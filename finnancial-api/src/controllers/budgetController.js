...
const create = asyncHandler(async (req, res) => {
  ...
  const budget = await prisma.$transaction(async (tx) => {
    ...
    return await Budget.create({ userId, categoryId: category_id, amountLimit: amount_limit, month, year }, tx);
  });

  logger.info({ userId, budgetId: budget.id }, 'Novo orçamento definido (Atomic)');
...
const update = asyncHandler(async (req, res) => {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    const budget = await Budget.findById(budgetId, tx);
    ...
    return await Budget.update(budgetId, req.body, tx);
  });

  logger.info({ userId, budgetId }, 'Orçamento atualizado (Atomic)');
  res.json(updated);
});
...
const remove = asyncHandler(async (req, res) => {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await Budget.remove(budgetId, tx);
  });

  logger.info({ userId, budgetId }, 'Orçamento removido (Atomic)');
  res.status(204).send();
});
