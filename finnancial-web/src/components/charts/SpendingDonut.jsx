...
export default function SpendingDonut({ data = [] }) {
  ...
  const chartData = data.map(d => ({ name: d.name, value: d.value, color: d.color || '#888780' }));
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px', marginTop: 16 }}>
        {chartData.map((item, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
            ...
            <span style={{ fontWeight: 500, color: 'var(--text-secondary)' }}>{formatCurrency(item.value)}</span>
          </div>
        ))}
      </div>
    </div>
