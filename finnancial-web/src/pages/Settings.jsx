...
function ProfileSection() {
  ...
  return (
    <div className={styles.profileSection}>
      ...
      {!isEditing && <p className={styles.hint}>O email é fixo e não pode ser alterado por questões de segurança.</p>}
    </div>
  );
}
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (account?.id) {
        ...
        await accountsApi.create({ name: name.trim(), type, balance: parseFloat(balance), color });
...
function DataSection() {
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = 'finnancial_export.json';
      a.click();
  ...
  return (
    <div>
      ...
      <div className={styles.dataCard}>
        <h4>Sobre o app</h4>
        <p className={styles.dataDesc}>Finnancial v2.1.0 — Backend: Node.js + Prisma + MongoDB, Frontend: React + Vite</p>
      </div>
    </div>
  );
}
