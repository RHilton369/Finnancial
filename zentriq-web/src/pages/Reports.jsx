                ...
                <BarChart data={monthlyData}>
                  <defs>
                    <linearGradient id="reportIncome" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#23B686" stopOpacity={1}/>
                      <stop offset="95%" stopColor="#1D9E75" stopOpacity={0.7}/>
                    </linearGradient>
                    <linearGradient id="reportExpense" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#EF5A58" stopOpacity={1}/>
                      <stop offset="95%" stopColor="#E24B4A" stopOpacity={0.7}/>
                    </linearGradient>
                  </defs>
                  ...
                  <Bar dataKey="income" fill="url(#reportIncome)" name="Receita" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="expense" fill="url(#reportExpense)" name="Despesa" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
              ...
              <table className={styles.monthlyTable}>
                ...
                <tbody>
                  {monthlyData.map(m => {
                    ...
                    const isZeroMonth = m.income === 0 && m.expense === 0;
                    return (
                      <tr key={m.month}
                      ...
                      >
                        <td>{m.label}</td>
                        <td className={m.income === 0 ? styles.zeroCell : styles.incomeCell}>
                          {formatCurrency(m.income)}
                        </td>
                        <td className={m.expense === 0 ? styles.zeroCell : styles.expenseCell}>
                          {formatCurrency(m.expense)}
                        </td>
                        <td className={m.balance === 0 ? styles.zeroCell : (m.balance > 0 ? styles.incomeCell : styles.expenseCell)}>
                          {formatCurrency(m.balance)}
                        </td>
                        <td>
                          <span className={`${styles.savingsBadge} ${isZeroMonth ? styles.grayBadge : (savingsRate >= 20 ? styles.green : savingsRate >= 0 ? styles.yellow : styles.red)}`}>
